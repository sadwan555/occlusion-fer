...
PrivateTest 只用于锁定后的最终评估。描述 48×48 灰度像素复制为三通道、
bilinear resize 到 112×112 和 ImageNet normalization。类别顺序固定为：
...
learning rate、weight decay、batch size、epoch budget、AMP 和 DataLoader workers。
best checkpoint 依据 clean PublicTest macro-F1 的严格提升，tie 保留更早 epoch。

#### 3.4 Occlusion protocol（后续阶段填充）
...
源图不原地修改，所有 checkpoint 使用相同 final masks。

#### 3.5 Training strategies（后续阶段填充）
...
1. clean baseline 三 seed 的 validation 与 final clean test；
2. clean-only checkpoint 在九个遮挡条件下的表现和 drop；
3. mixed checkpoint 的同条件表现；
4. 两种策略的逐条件、逐类和跨 seed 对比；
5. 失败、异常与负面结果。

只用 `final_test` 原始结果生成最终表。validation 用于解释 checkpoint 选择，
不能与 final test 混为一张主结果表。
...
| Figure 2 | 三类遮挡 × 三 ratio 示例网格 | 后续固定 mask 产物 | 等待遮挡阶段 |
| Figure 3 | train/validation loss 与 validation macro-F1 曲线 | 各 seed `history.csv` | clean 训练后可生成 |
| Figure 4 | clean final confusion matrix | `final_test/clean_confusion_matrix.csv` | 锁定后生成 |
| Figure 5 | 各遮挡条件 accuracy/macro-F1 与 clean drop | 后续跨 seed 汇总 CSV | 等待完整评估 |
| Figure 6 | 各类别在遮挡下的 F1 变化 | `*_per_class_metrics.csv` | 等待完整评估 |
| Table 1 | 数据、模型与训练设置 | `resolved_config.yaml`、metadata | clean run 后可填 |
| Table 2 | clean 三 seed 与 mean±std | clean metrics JSON | final clean 后可填 |
| Table 3 | clean-only 十条件结果 | 后续 condition metrics | 等待遮挡阶段 |
| Table 4 | mixed 与 clean-only 的 drop 对比 | 后续汇总 CSV | 等待 mixed 阶段 |
...
- `final_test/clean_metrics.json` → clean 结果与 Table 2；
- `*_per_class_metrics.csv` → 逐类表和 Figure 6；
- `*_confusion_matrix.csv` → Figure 4；
- `*_predictions.csv` → 配对 condition drop、置信度和错误案例审计；
...
- 三个 seed 全部完成并全部报告；
- PrivateTest 只在 clean/mixed、九个遮挡条件和 final masks 全部锁定后运行；
